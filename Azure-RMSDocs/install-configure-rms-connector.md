...
1.  RMS 커넥터를 실행할
2.  컴퓨터(최소 2대)를 식별합니다. 이러한 컴퓨터는 필수 조건에 나열되어 있는 최소 사양을 충족해야 합니다.
...
2.  [Microsoft 다운로드 센터](https://go.microsoft.com/fwlink/?LinkId=314106)에서 RMS 커넥터의 원본 파일을 다운로드합니다.
...
-   커넥터에 대한 보안 토큰의 집합은 Azure RMS로 작업 권한을 부여합니다. 이러한 토큰을 Azure RMS에서 다운로드하고 레지스트리의 로컬 컴퓨터에 설치합니다. 데이터 보호 애플리케이션 프로그래밍 인터페이스(DPAPI) 및 로컬 시스템 계정 자격 증명을 사용하여 보호합니다.
...
DNS에서 이름을 만들고 IP 주소에 대해 구성한 후 트래픽을 커넥터 서버로 전송하는 해당 주소의 부하 분산을 구성합니다. 이렇게 하기 위해 Windows Server의 NLB(네트워크 부하 분산) 기능 등 모든 IP 기반 부하 분산 장치를 사용할 수 있습니다. 자세한 내용은 [부하 분산 배포 가이드](https://technet.microsoft.com/library/cc754833%28v=WS.10%29.aspx)를 참조하세요.
...
> -   독립 CA를 사용하거나 다른 회사에서 이 인증서를 구입하는 경우에는 TechNet의 [웹 서버(IIS)](https://technet.microsoft.com/library/cc753433%28v=ws.10%29.aspx) 문서 라이브러리에서 [인터넷 서버 인증서 구성(IIS 7)](https://technet.microsoft.com/library/cc731977%28v=ws.10%29.aspx)을 참조하세요.
> - 인증서를 사용하도록 IIS를 구성하려면 TechNet의 [Web Server (IIS)](https://technet.microsoft.com/library/cc753433%28v=ws.10%29.aspx)(웹 서버(IIS)) 문서 라이브러리에서 [Add a Binding to a Site (IIS 7)](https://technet.microsoft.com/library/cc731692.aspx)(사이트에 바인딩 추가(IIS 7))를 참조하세요.
...
이러한 파일을 아직 다운로드하지 않았으면 [Microsoft 다운로드 센터](https://go.microsoft.com/fwlink/?LinkId=314106)에서 다운로드할 수 있습니다.
