...
문서와 이메일을 분류하고 보호하는 가장 쉬운 방법은 Office 데스크톱 앱(**Word**, **Excel**, **PowerPoint**, **Outlook**) 내에서 만들고 편집하는 경우입니다. 
...
레이블에서 문서에 보호를 적용하면 보호된 문서는 SharePoint 또는 OneDrive에서 저장하는 데 적합하지 않습니다. 이러한 위치는 보호된 파일에 대해 공동 작성, Office Online, 검색, 문서 미리 보기, 썸네일 및 eDiscovery를 지원하지 않습니다. 
...
2. **Microsoft Azure Information Protection** 대화 상자에서 다음을 지정합니다.
    ...
    - **사용 권한 선택**: 자기만 액세스할 수 있도록 파일을 보호하려면 **나만**을 선택합니다. 그렇지 않은 경우 사람들에게 부여할 액세스 레이블을 선택합니다.
...
관리자 가이드에는 [Azure Information Protection 클라이언트에서 지원하는 파일 형식](client-admin-guide-file-types.md)에 지원되는 파일 형식의 전체 목록과 자동으로 제외된 파일 및 폴더 목록이 포함됩니다.
...
2. **분류 및 보호 - Azure Information Protection** 대화 상자에서 Office 응용 프로그램에서와 같이 레이블을 사용합니다. 그러면 관리자가 정의한 분류와 보호가 설정됩니다. 
    ...
    - 레이블을 선택할 수 없는 경우(흐리게 표시): 선택한 파일이 분류를 지원하지 않지만 사용자 지정 권한으로 보호할 수 있습니다(3단계). 예를 들면 다음과 같습니다.
...
4. 사용자 지정 권한 옵션을 선택한 경우 다음 항목을 지정하세요.
    ...
    - **사용 권한 선택**: 선택한 하나 이상의 파일을 보호할 때 사용자에게 제공할 액세스 권한 수준을 선택합니다.
