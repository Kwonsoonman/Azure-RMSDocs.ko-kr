...
> [!IMPORTANT]
> *AES 256* 보호된 파일 사용은 [2014년 10월 릴리스](release-notes-rtm.md)부터 지원되었습니다. 2014년 10월 이전 버전의 SDK로 빌드한 애플리케이션을 실행하는 경우 이 업데이트를 사용하면 애플리케이션이 중단됩니다. 빌드하는 애플리케이션의 고객이 업데이트된 SDK를 사용 중이거나 최신 버전의 애플리케이션으로 즉시 업데이트할 의향이 있는지 확인합니다.
...
*IPC\_LI\_DEPRECATED\_ENCRYPTION\_ALGORITHMS* 플래그는 더 이상 API에 노출되지 않습니다. 즉, 이후 애플리케이션에서 이 플래그를 참조할 경우 더 이상 컴파일되지 않지만 API 코드에서 비공개로 플래그가 적용되므로 이 플래그를 사용하여 이미 빌드된 애플리케이션은 계속 작동합니다.
