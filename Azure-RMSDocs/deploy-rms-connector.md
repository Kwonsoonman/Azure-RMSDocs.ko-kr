...
테넌트 키를 관리하는 이 시나리오에 대해 자세히 알아보려면 [Azure Information Protection 테넌트 키 계획 및 구현](plan-implement-tenant-key.md)을 참조하세요.
