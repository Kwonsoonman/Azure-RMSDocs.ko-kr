...
6. 또한 **Office 앱에 Information Protection 표시줄 표시합니다.**가 **설정**으로 설정되어 있는지 확인합니다.
...
17. 이 **정책: 전역**(정책: 전역) 블레이드에서 **저장**을 클릭합니다. 이 작업을 확인하라는 메시지가 표시되면 **확인**을 클릭합니다.
...
3. **분류** > **레이블** 메뉴 옵션의 **Azure Information Protection - 레이블** 블레이드에서 만든 **금융** 레이블에 대한 상황에 맞는 메뉴(**...**)를 선택합니다.
