...
2. AD RMS 게시 엔드포인트에서 Office 2016 사용자에 대한 거부 권한을 설정합니다.
    ...
    b. **기본 웹 사이트** > **_wmcs** > **라이선스** > **licensing.asmx**로 이동합니다.

    c. **licensing.asmx** > **속성** > **편집**을 마우스 오른쪽 단추로 클릭합니다.

    d. **licensing.asmx에 대한 권한** 대화 상자에서 모든 사용자에 대한 리디렉션을 설정하려는 경우에 **사용자**를 선택하거나 **추가**를 클릭한 다음, 리디렉션할 사용자를 포함하는 그룹을 지정합니다.
    ...
    f. 이 구성이 예상대로 작동하는지 확인하려면 브라우저에서 직접 licensing.asmx 파일로 연결을 시도합니다. SRV 레코드를 찾을 Office 2016을 실행하는 클라이언트를 트리거하는 다음과 같은 오류 메시지가 나타납니다.
