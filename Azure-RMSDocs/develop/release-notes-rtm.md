...
-   **서비스 앱 및 클라우드 기반 RMS** - [**IPC\_CREDENTIAL\_SYMMETRIC\_KEY**](/rights-management/sdk/2.1/api/win/ipc_credential#msipc_ipc_credential_symmetric_key)에는 대칭 키, **AppPrincipalId** 및 **TenantBposId**의 세 가지 정보가 필요합니다. 이와 관련된 항목이 획득한 이 정보의 처리 지침을 제공하도록 업데이트되었습니다. 이 업데이트에 대해서는 [Enable your service application to work with cloud based RMS](how-to-use-file-api-with-aadrm-cloud.md)(서비스 응용 프로그램이 클라우드 기반 RMS를 사용할 수 있도록 설정) 항목의 업데이트된 버전을 참조하세요..

## 2015년 4월 업데이트

-   이제 새로운 API 집합을 통해 **문서 추적**을 사용할 수 있습니다. 자세한 내용은 [Tracking Content](tracking-content.md)(콘텐츠 추적) 항목을 참조하세요..
-   **암호화 유형** - 이제 암호화 패키지 선택에 대해 API 수준 제어를 지원합니다. 자세한 내용은 [Working with encryption](working-with-encryption.md)(암호화 작업) 항목을 참조하세요..

    **참고** **IPC\_LI\_DEPRECATED\_ENCRYPTION\_ALGORITHMS** 플래그는 더 이상 API에 노출되지 않습니다. 즉, 이후 앱에서 이 플래그를 참조할 경우 더 이상 컴파일되지 않지만 API 코드에서 비공개로 플래그가 적용되므로 이미 빌드된 앱은 계속 작동합니다. 플래그를 변경하기만 하면 사용되지 않는 이전 암호화 알고리즘 플래그를 여전히 활용할 수 있습니다. 자세한 내용은 [Working with encryption](working-with-encryption.md)(암호화 작업) 항목을 참조하세요..

     

-   [**API 모드 값**](/rights-management/sdk/2.1/api/win/api%20mode%20values#msipc_api_mode_values_IPC_API_MODE_SERVER)이 **IPC\_API\_MODE\_SERVER**인 **서버 모드 응용 프로그램**에는 응용 프로그램 매니페스트가 더 이상 필요하지 않습니다. 프로덕션 RMS 서버에서 응용 프로그램을 테스트할 수 있으며, 프로덕션 환경으로 전환할 때 프로덕션 라이선스를 얻지 않아도 됩니다. 서버 모드 응용 프로그램에 대한 자세한 내용은 [Application types](application-types.md)(응용 프로그램 유형) 항목을 참조하세요..
...
-   **지원되는 보호된 파일(pfile) 크기 증가** - 이제 1GB보다 큰 pfile 크기를 지원합니다. pfile에 대한 자세한 내용은 [Support File Formats](supported-file-formats.md)(지원되는 파일 형식) 항목을 참조하세요..
...
-   SDK의 파일 API 구성 요소에 대한 기본 동작이 업데이트되었습니다. 자세한 내용은 [File API configuration](file-api-configuration.md)(파일 API 구성) 항목을 참조하세요..
-   새로운 기능인 메일 알림은 개발자 노트 항목인 [Enabling email notification](how-to-enable-email-notification.md)(메일 알림 사용)에서 설명합니다..
...
-   이제 **IPC\_LI\_CONTENT\_ID** 속성을 통해 **콘텐츠 ID**를 쓸 수 있습니다. 자세한 내용은 [**License property types**](/rights-management/sdk/2.1/api/win/License%20property%20types#msipc_license_property_types_IPC_LI_APP_SPECIFIC_DATA)(라이선스 속성 형식) 항목을 참조하세요..
-   **프로덕션 매니페스트 요구 사항** - 서버 모드에서 RMS 사용 응용 프로그램/서비스를 실행하는 경우에는 더 이상 매니페스트가 필요하지 않습니다. 자세한 내용은 [Application types](application-types.md)(응용 프로그램 유형) 항목을 참조하세요..
-   **설명서 업데이트**

    **재구성됨** - [ - 환경 설정 및 응용 프로그램 테스트 단계 순서를 명확하게 설명하기 위해 사용 방법](how-to-use-msipc.md)이 재구성되었습니다.

    **테스트 모범 사례** - Azure RMS를 사용하여 테스트하기 전에 온-프레미스 서버를 사용하는 지침이 추가되었습니다. 자세한 내용은 [Enable your service application to work with cloud based RMS](how-to-use-file-api-with-aadrm-cloud.md)(서비스 응용 프로그램이 클라우드 기반 RMS를 사용할 수 있도록 설정) 항목을 참조하세요..
...
-   **모든 파일 형식에 대한 기본 지원**
    ...
    지원되는 파일 형식에 대한 자세한 내용은 [File API configuration](file-api-configuration.md)(파일 API 구성) 항목을 참조하세요..
...
-   **파일 API 통합**
    ...
    기본 보호를 사용하여 Microsoft Office 파일, PDF(Portable Document Format) 파일 및 선택한 다른 파일 형식을 보호할 수 있습니다. 기본 보호를 사용하여 보호할 수 있는 파일 형식의 전체 목록은 [File API configuration](file-api-configuration.md)(파일 API 구성) 항목을 참조하세요..

    RMS 보호된 파일 형식(PFile)을 사용하여 시스템 파일 및 Office 파일을 제외한 모든 파일을 보호할 수 있습니다.

-   **문제**: 라이선스를 처음부터 만드는 경우 소유권 권한을 명시적으로 부여해야 합니다.

    **해결 방법**: [**IpcCreateLicenseFromScratch**](/rights-management/sdk/2.1/api/win/functions#msipc_ipccreatelicensefromscratch)를 사용하여 라이선스를 처음부터 만드는 경우 응용 프로그램에서 라이선스 소유자에게 **소유자** 권한을 명시적으로 추가해야 합니다. 자세한 내용은 [Add explicit owner rights](add-explicit-owner-rights.md)(명시적 소유자 권한 추가) 항목을 참조하세요..
...
-   **문제**: 응용 프로그램이 해당 핸들을 사용하여 동일한 창에 대해 [**IpcProtectWindow**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcprotectwindow) 또는 [**IpcUnprotectWindow**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcunprotectwindow)를 두 번 호출하는 경우 RMS SDK 2.1에서 **HRESULT**에 오류를 반환합니다..
    ...
    **해결 방법**: 이 문제에 대한 구체적인 지침은 [**IpcProtectWindow**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcprotectwindow) 및 [**IpcUnprotectWindow**](/rights-management/sdk/2.1/api/win/functions#msipc_ipcunprotectwindow)에서 설명 섹션을 참조하세요..
