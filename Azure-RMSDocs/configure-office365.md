...
Office 365는 기본적으로 Azure Information Protection의 Azure Rights Management 서비스를 지원하므로 Word, Excel, PowerPoint, Outlook 및 웹의 Outlook과 같은 응용 프로그램에 대해 IRM(정보 권한 관리) 기능을 지원하기 위해 클라이언트 컴퓨터를 구성할 필요가 없습니다. 모든 사용자는 Rights Management 자격 증명으로 Office 응용 프로그램에 로그인해야 합니다. 그러면 파일과 메일을 보호하고 다른 사용자가 보호한 파일과 메일을 사용할 수 있습니다.
...
4. [Get-IRMConfiguration](https://technet.microsoft.com/library/dd776120(v=exchg.160\).aspx) 명령을 실행하여 보호 서비스의 Exchange Online 구성을 봅니다.
    ...
    - AzureRMSLicensingEnabled를 **False**로 설정하면 Azure Rights Management 서비스에 Exchange Online을 사용하도록 설정하는 다음 명령을 실행합니다. `Set-IRMConfiguration -AzureRMSLicensingEnabled $true`
...
> [!NOTE]
> IRM으로 보호되는 SharePoint용 라이브러리 및 비즈니스용 OneDrive에는 새 OneDrive 동기화 클라이언트(OneDrive.exe)의 최신 버전 및 [Microsoft 다운로드 센터에서 제공되는 RMS 클라이언트](https://www.microsoft.com/en-us/download/details.aspx?id=38396) 버전이 필요합니다. Azure Information Protection 클라이언트를 설치한 경우에도 이 버전의 RMS 클라이언트를 설치하세요. 이 배포 시나리오에 대한 자세한 내용은 [엔터프라이즈 환경에서 새 OneDrive 동기화 클라이언트 배포](https://support.office.com/article/Deploy-the-new-OneDrive-sync-client-in-an-enterprise-environment-3f3a511c-30c6-404a-98bf-76f95c519668)를 참조하세요.
