...
- Azure Rights Management 서비스의 위임된 관리에 대한 테넌트에서 확인된 도메인에 있는 Azure AD(메일 주소 유무는 무관)의 모든 그룹 유형을 사용할 수 있습니다.
...
그런 다음 **ProxyAddresses** 열이 채워져 있는지 확인합니다. 채워져 있으면 이 열에 있는 메일 값을 사용하여 사용자에게 Azure Rights Management 서비스에 대한 권한을 부여할 수 있습니다. 
...
> 
> `Get-MsolGroup | select DisplayName, ProxyAddresses | Export-Csv -Path UserAccounts.csv`
...
사용자와 그룹을 Azure Information Protection에 사용할 수 있는 것을 확인하고 문서 및 메일을 보호할 준비를 완료하고 나면, Rights Management 서비스를 활성화하여 데이터 보호 서비스를 사용하도록 설정합니다. 자세한 내용은 [Azure Rights Management 활성화](../deploy-use/activate-service.md)를 참조하세요.
