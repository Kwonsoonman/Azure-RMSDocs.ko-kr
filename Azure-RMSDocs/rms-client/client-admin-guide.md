...
---
# Azure Information Protection 클라이언트 관리자 가이드
...
## 사용자를 위해 Azure Information Protection 클라이언트를 설치하는 방법
...
클라이언트를 설치하기 위한 옵션에는 다음 세 가지가 있습니다.
...
2. .msi 파일을 실행할 각 컴퓨터에 대해 다음과 같은 소프트웨어 종속성이 유지되는지 확인해야 합니다. 예를 들어 이러한 종속성을 .msi 버전의 클라이언트로 패키징하거나 다음 종속성을 충족하는 컴퓨터에만 배포합니다.
