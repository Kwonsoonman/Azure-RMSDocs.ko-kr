...
Azure Information Protection의 구독 또는 Azure Rights Management 서비스를 포함하는 Office 365 구독을 얻을 때, 조직에서 권한 있는 사용자만 액세스할 수 있도록 제한하는 두 개의 기본 템플릿이 테넌트용으로 자동 생성됩니다. 이 두 개의 템플릿에는 다음 제한이 있습니다. 
...
이러한 템플릿을 사용하면 사용자가 조직의 중요한 데이터 보호를 즉시 쉽게 시작할 수 있습니다. 이러한 템플릿은 Azure Information Protection 레이블과 함께 사용하거나 Rights Management 템플릿을 사용할 수 있는 [응용 프로그램과 서비스](../understand-explore/applications-support.md)에서 단독으로 사용할 수 있습니다.
...
최근에 Azure Information Protection의 구독을 얻은 경우 기본 템플릿의 이름은 다음과 같습니다.

- **Confidential \ All Employees** - 보호된 콘텐츠의 읽기 또는 수정 권한이 있습니다.

- **Highly Confidential \ All Employees** - 보호된 콘텐츠를 읽기 전용으로 볼 수 있습니다.

이전에 Azure Information Protection 구독을 얻었거나 Azure Information Protection 구독이 없었지만 Azure Rights Management를 포함하는 Office 365 구독이 있으면 기본 템플릿의 이름은 다음과 같습니다.
...
- 현재, Azure 클래식 포털 또는 PowerShell을 사용하여 여러 언어에 대해 구성된 템플릿은 이름 및 설명을 해당 언어로 표시하지 않고 그대로 유지됩니다.

- 템플릿을 레이블로 변환하거나 연결하면 다른 레이블에서 더 이상 사용할 수 없습니다.

- **템플릿** 컨테이너에서 새 템플릿을 만들지 않습니다. 대신 **보호** 설정이 있는 레이블을 만들고 **보호** 블레이드에서 사용 권한과 설정을 구성합니다. 전체 지침은 [새 템플릿을 만들려면](#to-create-a-new-template)을 참조하세요.
...
- 원본 템플릿이 더 이상 **템플릿**에 표시되지 않으므로 레이블에 보호를 구성할 때 미리 정의된 템플릿으로 선택할 수 없습니다. Azure 포털에서 이 템플릿을 편집하기 위해 이제 템플릿을 변환할 때 만든 레이블을 편집합니다. 템플릿은 Azure 권한 관리 서비스에 계속 사용할 수 있으며 [PowerShell 명령](administer-powershell.md)을 사용하여 계속 관리할 수 있습니다.  
