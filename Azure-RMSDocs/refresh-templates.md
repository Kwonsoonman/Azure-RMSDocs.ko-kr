...
1.  레지스트리 편집기를 사용하여 다음 레지스트리 값 중 하나를 만들고 설정합니다.
    ...
    - 업데이트 빈도(일)를 설정하려면(최소 1일):  이름이 **TemplateUpdateFrequency**인 새 레지스트 값을 만들고 데이터의 정수 값을 정의합니다. 그러면 다운로드한 템플릿에 대한 모든 변경 사항을 다운로드하는 주기가 일 단위로 지정됩니다. 다음 정보를 사용하여 새 레지스트리 값을 만들기 위한 레지스트리 경로를 확인합니다.
    ...
    - 업데이트 주기를 초 단위로 설정하려면(최소 1초):  이름이 **TemplateUpdateFrequencyInSeconds**인 새 레지스트 값을 만들고 데이터의 정수 값을 정의합니다. 그러면 다운로드한 템플릿에 대한 모든 변경 사항을 다운로드하는 주기가 초 단위로 지정됩니다. 다음 정보를 사용하여 새 레지스트리 값을 만들기 위한 레지스트리 경로를 확인합니다.
...
1.  레지스트리 편집기에서 **LastUpdatedTime** 값의 데이터를 삭제합니다. 예를 들어 데이터가 **2015-04-20T15:52**로 표시된다면 2015-04-20T15:52를 삭제하여 아무 데이터도 표시되지 않게 합니다. 다음 정보를 참조하여 이 레지스트리 값 데이터를 삭제할 레지스트리 경로를 찾습니다.
    ...
    **값:** LastUpdatedTime
    ...
    > Azure RMS에 대해 [Get-AadrmConfiguration](/powershell/module/aadrm/get-aadrmconfiguration) cmdlet을 실행합니다. Azure RMS용 Windows PowerShell 모듈을 아직 설치하지 않은 경우 [AADRM PowerShell 모듈 설치](install-powershell.md)를 참조하세요.
    >
