...
> [!IMPORTANT] 두 가지 경우 모두에서 수동으로 모든 필수 구성 요소를 설치하고 Rights Management를 사용하도록 Exchange, SharePoint 및 파일 분류 인프라를 구성해야 합니다.
...
> [!IMPORTANT] 이 도구를 실행할 때는 RMS 커넥터 서비스를 실행하는 단일 서버 이름이 아니라, 조직의 부하가 분산된 RMS 커넥터 이름을 지정해야 합니다.
...
> [!WARNING] 여러 버전의 MSIPC 2.1 클라이언트가 있으므로 1.0.2004.0 이상 버전이 있는지 확인합니다.
>
