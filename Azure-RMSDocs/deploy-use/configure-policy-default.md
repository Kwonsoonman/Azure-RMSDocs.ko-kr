...
이 버전의 기본 정책은 2017년 3월 21일에 생성되었습니다.

이 정책의 설명에서는 보호가 필요한 데이터를 참조하고 데이터 추적 및 해지도 참조합니다. 정책에서는 이러한 레이블에 대해 이 보호를 구성하지 않으므로 이 설명을 준수하기 위한 추가 단계를 수행해야 합니다. 예를 들어 Azure RMS 보호를 적용하거나 DLP(데이터 손실 방지) 솔루션을 사용하도록 레이블을 구성합니다. 문서 추적 사이트를 사용하여 문서를 추적하고 해지하려면 먼저 문서가 Azure RMS로 보호되어야 합니다. 
